function params = SimulateOdorA_withoutOmission(simulation_index,Weber_fraction,jitter,belief_impaired,weights,learning_rate)
...
ISIdistribution = round(full_ISIpdf*nTrials);
trialtype=[];
for i=1:length(ISIs)
...
% probability
T_blur(30,30)=(1-ITIhazard);
T_blur(30,1)=(ITIhazard);
%%
...
%stimulus onset
O(30,1,:) = [0 1 0];
O(30,30,2) = 0 *(1-belief_impaired) + 0 *(belief_impaired); %omission trial

%ITI
O(30,30,1) = 1 *(1-belief_impaired) + 1 *(belief_impaired);
...
O((T_blur(1:29,30))>0.0001,30,3)=1;

%%
% Run TD learning
results = TDlearning(x,O,T_blur,weights,belief_impaired,Weber_fraction,learning_rate);

save(strcat('A100_results',num2str(simulation_index),'.mat'))
